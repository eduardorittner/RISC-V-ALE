...
export class UI_Helper {
  constructor(intro) {
    ...
    document.getElementById("assistant_run_button").onclick =
      this.runTests.bind(this);
    this.sleep = (milliseconds) => {
      return new Promise((resolve) => setTimeout(resolve, milliseconds));
    };
    this.final_result = (_) => "";
    document.getElementById("assistant_test_list").insertAdjacentHTML(
      "afterend",
      `
  ...
  async runTests() {
    ...
    for (const t in this.tests) {
      ...
      if (result) {
        ...
        html_item.setAttribute(
          "onclick",
          `alert(unescape("${escape(this.item_log)}"))`,
        );
...
export class Assistant_Script {
  constructor() {
    ...
    this.wait_for_output = async function ({
      msg = "",
      size = 1,
      fh = 1,
      timeout = 5000,
      ...
      return new Promise((resolve) => {
        this.stdioCallback = function () {
          ...
          if (
            stdioHandler.slice(bufferStart).includes(msg) &&
            stdioHandler.length - bufferStart >= size
          )
            resolve();
        }.bind(this);
  ...
  simple_equality_test(
    stdin,
    expected_output,
    { timeout = 5000, compare_function = (x, y) => x == y } = {},
  ) {
    return async function () {
      ...
      this.ui.log(
        `Input: ${stdin} Expected: ${expected_output} Result: ${result}`,
      );
      this.log(
        `Input: ${stdin} Expected: ${expected_output} Result: ${result}`,
      );
